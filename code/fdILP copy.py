...
def genConsStrNOTUSEDWTHY(domains,interdom,scales,compcount,cliques):
    """generates lp relaxation constraints
    Args:
       domains: set of domains
       interdom: interacting domain pairs
...
def genObjStr2NOTUSED(freqmat,node2dom,scales,compcount):
    """generates objective function string
    Args:
       freqmat: frequency matrix
       node2dom: node to domain index mapping
    ...
    singles = []
    for index1 in xrange(np.shape(freqmat)[0]):
        for index2 in xrange(np.shape(freqmat)[1]):
            domset = node2dom[index1].intersection(node2dom[index2])
            seenset = [(dom,comp,scale) for dom in domset for scale in scales for comp in xrange(compcount)]        
            singles.extend([" - {0} x{1}_{2}_{3} ".format(2*freqmat[index1,index2]*scale,dom,comp,scale) for dom,comp,scale in seenset])
    objstr += " ".join(singles)
    quads = []
    quaddict = {}        
    for index1 in xrange(np.shape(freqmat)[0]):
        ...
        for index2 in xrange(np.shape(freqmat)[1]):
            domset = node2dom[index1].intersection(node2dom[index2])
            seenset = [(dom,comp,scale) for dom in domset for scale in scales for comp in xrange(compcount)]
            quads.extend([" {0} x{1}_{2}_{3} * x{4}_{5}_{6} ".format(2*scale1*scale2,dom1,comp1,scale1,dom2,comp2,scale2) for dom1,comp1,scale1 in seenset for dom2,comp2,scale2 in seenset])
    if len(quads) > 0:
       objstr += " + [ " + " + ".join(quads) + " ] "
    return objstr,set()
...
def runILP(freqmat,domains,scales,compcount,params):
    ...
    cliques = findCliqueDecomp(domains,interdom)
    #interdom2 = []
    ...
    #cliques2 = findCliqueDecomp(domains,interdom2)
